...
가격:<input type="number" name="prodPrice" min="10" step="10" value="${goodsCommand.prodPrice }"/>
...
배송비:<input type="number" name="prodDelFee"  value="${goodsCommand.prodDelFee }"/>
...
카테고리:<select name="ctgr">
		...
		<option value="car"
		<c:if test="${goodsCommand.ctgr == 'car' }">selected</c:if>>붕붕이용품</option>
	</select><br/>
상세내용:<textarea rows="5" cols="50" name="prodDetail">${goodsCommand.prodDetail }</textarea>
<form:errors path="prodDetail"/><br/>
이미지	
<c:forTokens items="${goodsCommand.prodImage } " delims="," var="prodImage" ><!-- <<구분자  -->
<p>
...
<input type="button" value="삭제" onclick="javascript:location.href='goodsDel?prodNum=${goodsCommand.prodNum }'"/>
