...
카테고리: <c:if test="${goodsCommand.ctgr == 'wear' }">의류</c:if>
		<c:if test="${goodsCommand.ctgr == 'cosmetic' }">피부에게양보!</c:if>
		<c:if test="${goodsCommand.ctgr == 'food' } ">냠냠먹을것</c:if>
		<c:if test="${goodsCommand.ctgr == 'car' } ">붕붕이용품</c:if>
		<br/>
상품번호:${goodsCommand.prodNum }<br/>
상품명:${goodsCommand.prodName }<br/>
상품가격:<fmt:formatNumber value="${goodsCommand.prodPrice }" type="currency"/>
<br/>
상품규격:${goodsCommand.prodCapacity }<br/>
